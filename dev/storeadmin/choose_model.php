...
if(isset($_POST['item_to_accept'])) {
    $accept_id = $_POST['item_to_accept'];
         $sql = mysql_query("UPDATE products SET status = '1' WHERE id = '$accept_id';")or die(mysql_error());
        header("location:accept_products.php");
        exit();
}
...
$id_to_change = $_POST{'id_to_change'};
  ...
  header("location:choose_model.php");
  exit();
}
...
$dynamic_list = "";
$sql = mysql_query("SELECT * FROM products WHERE model_id = 0 AND model_wears = '' OR model_wears = 'stærð' ORDER BY date_added DESC LIMIT 30 "); //SELECT * ÞÝÐIR SELECT ALL

...
if(isset($_GET['id'])){
    ...
    $id = htmlspecialchars(mysql_real_escape_string($_GET{"id"}));
        $sql = mysql_query("SELECT * FROM stores WHERE id='$id' LIMIT 1"); //SELECT * ÞÝÐIR SELECT ALL
        $productCount = mysql_num_rows($sql);
        if($productCount>0){
            ...
            mysql_query("UPDATE stores SET visited = visited + 1 WHERE id='$id' LIMIT 1");
        } else {
...
$count_sql = mysql_query("SELECT * FROM products WHERE price > 1 AND store = '$storename' AND status = 1"); //SELECT * ÞÝÐIR SELECT ALL
...
if($pages !=1) {
  ...
  if($page != 1){
    $prev = $page - 1;
    $pagination .= '<a href="/verslun/'.$id.'/'.$prev.'"> < </a>';
  }else {
    $pagination .= '<';
  }
  if($pages > 1) {
    for($i = 1; $i <= $pages; $i++) {
        ...
        $pagination .= '<a href="/verslun/'.$id.'/'.$i.'" class="'.$active.'"> '.$i.' </a>';
    }
  }
  if($page != $pages){
    $next = $page + 1;
    $pagination .= '<a href="/verslun/'.$id.'/'.$next.'"> > </a>';
  }else {
    $pagination .= '>';
  }
...
$dynamic_list = "";
$sql = mysql_query("SELECT * FROM products WHERE store = '$storename' AND status = 1 ORDER BY date_added DESC $limit") ; //SELECT * ÞÝÐIR SELECT ALL
if($sql){
    while ($row=mysql_fetch_array($sql)) {
        $id = $row{"id"};
        ...
        $subcategory = $row{"subcategory"};
                /*Gera Athuganir og lagfæra breytur*/
        if ($x>0){
...
<li> 
    ...
    $dynamic_list = "<p style='font-size:25px;'>".$storename . " hefur ekki enn sett inn vörur. <br/><br/> Komdu aftur á morgun og sjáðu hvort það sé ekki komið þá!</p>";
} 
?>
...
if($productCount>0){
    while ($row=mysql_fetch_array($sql)) {
        ...
        $category_list_menn .= '
<a href="/flokkur/' . $cid . '"><li>' . $list_category_name . '</li></a>
';
    } 
} else {

...
//--------------------
    ...
    $sql = mysql_query(" SELECT * FROM orders WHERE date_added BETWEEN '$date1' AND '$date2' AND store = '$storename' ORDER BY id DESC"); //SELECT * ÞÝÐIR SELECT ALL 
    } else {
    $sql = mysql_query(" SELECT * FROM orders WHERE MONTH(`date_added`) = MONTH(CURDATE()) AND YEAR(`date_added`) = YEAR(CURDATE()) AND store = '$storename' ORDER BY id DESC"); //SELECT * ÞÝÐIR SELECT ALL
    }
...
<p style=" margin: 0;"><?php echo $kennitala; ?></p>
<p>Reikningsskil</p>
<p style=" margin: 0;"><?php echo $currentdate;?></p>
    <span class="no-print"style="text-align:center;">
            ...
            <hr/>
            <h3 align='center'> <?php if($manager!="admin") {echo $storename;} else {echo "Allar Búðir";}?></h3>

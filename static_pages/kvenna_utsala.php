...
$count_sql = mysql_query("SELECT * FROM products WHERE category = 'konur' AND status = 1 AND dcount > 0"); //SELECT * ÞÝÐIR SELECT ALL
...
if($pages !=1) {
  ...
  if($page != 1){
    $prev = $page - 1;
    $pagination .= '<a href="/konur_utsala/'.$prev.'"> < </a>';
  }else {
    $pagination .= '<';
  }
  if($pages > 1) {
    for($i = 1; $i <= $pages; $i++) {
        ...
        $pagination .= '<a href="/konur_utsala/'.$i.'" class="'.$active.'"> '.$i.' </a>';
    }
  }
  if($page != $pages){
    $next = $page + 1;
    $pagination .= '<a href="/konur_utsala/'.$next.'"> > </a>';
  }else {
    $pagination .= '>';
  }
...
$dynamic_list = "";
$sql = mysql_query("SELECT * FROM products WHERE category = 'konur' AND status = 1 AND dcount > 0 ORDER BY date_added DESC $limit"); //SELECT * ÞÝÐIR SELECT ALL
if($sql){
    while ($row=mysql_fetch_array($sql)) {
        $id = $row{"id"};
        ...
        if (isset($x)&&$x>0){
          ...
          $oldprice = '<span class="oldprice">' . $price . '</span>';
        } else {
          $discount_display = '';
          $oldprice = '';
        }
...
<li> 
    ...
    $dynamic_list = "Það eru engar vörur á útsölu";
} 
?>
...
<div id="main"> 
                  ...
                  <div class="max-width">
                  <h1> Útsala! </h1>
                  <p> Finndu bestu tilboðin á Íslandi og kauptu hlutina sem þú elskar, á betra verði.</p>
                  </div>
                </div>

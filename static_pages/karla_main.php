...
$count_sql = mysql_query("SELECT * FROM products WHERE category = 'menn' AND status = 1"); //SELECT * ÞÝÐIR SELECT ALL
...
if($pages !=1) {
  ...
  if($page != 1){
    $prev = $page - 1;
    $pagination .= '<a href="/karlar/'.$prev.'"> < </a>';
  }else {
    $pagination .= '<';
  }
  ...
  if($pages > 1) {
    for($i = 1; $i <= $pages; $i++) {
        ...
        $pagination .= '<a href="/karlar/'.$i.'" class="'.$active.'"> '.$i.' </a>';
    }
  }
  if($page != $pages){
    $next = $page + 1;
    $pagination .= '<a href="/karlar/'.$next.'"> > </a>';
  }else {
    $pagination .= '>';
  }
...
$dynamic_list = "";
$sql = mysql_query("SELECT * FROM products WHERE category = 'menn' AND status = 1 ORDER BY date_added DESC $limit"); //SELECT * ÞÝÐIR SELECT ALL
if($sql){
    while ($row=mysql_fetch_array($sql)) {
        $id = $row{"id"};
...
<div id="main"> 
                  ...
                  <div class="max-width">
                  <h1> <?php echo 'Menn'; ?> </h1>
                  <p> Allt það nýjasta á Íslandi fyrir menn.</p>
                  </div>
                </div>
    <ul id="products">

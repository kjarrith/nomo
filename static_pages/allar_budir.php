...
$count_sql = mysql_query("SELECT * FROM products WHERE status = 1"); //SELECT * ÞÝÐIR SELECT ALL
...
$dynamic_list = "";
$sql = mysql_query("SELECT * FROM products WHERE status = 1 ORDER BY date_added DESC $limit"); //SELECT * ÞÝÐIR SELECT ALL
if($sql){
    while ($row=mysql_fetch_array($sql)) {
        $id = $row{"id"};
...
<script type="text/javascript">
  function getSearch(value) {
      $.post("closerlook",{color:value}, function(data){
        $("#results").html(data);
      });
  }
</script>
...
<div id="main"> 
                  ...
                  <div class="max-width">
                  <h1> Allt frá öllum </h1>
                  <p> Allt það nýjasta á Íslandi.</p>
                  </div>
                </div>
    <ul>
           <div id="feedname">

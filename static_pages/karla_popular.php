...
$count_sql = mysql_query("SELECT * FROM products WHERE category = 'menn' AND status = 1 LIMIT 30"); //SELECT * ÞÝÐIR SELECT ALL
...
if($pages !=1) {
  ...
  if($page != 1){
    $prev = $page - 1;
    $pagination .= '<a href="/karlar_top/'.$prev.'"> < </a>';
  }else {
    $pagination .= '<';
  }
  ...
  if($pages > 1) {
    for($i = 1; $i <= $pages; $i++) {
        ...
        $pagination .= '<a href="/karlar_top/'.$i.'" class="'.$active.'"> '.$i.' </a>';
    }
  }
  if($page != $pages){
    $next = $page + 1;
    $pagination .= '<a href="/karlar_top/'.$next.'"> > </a>';
  }else {
    $pagination .= '>';
  }
...
$dynamic_list = "";
$sql = mysql_query("SELECT * FROM products WHERE category = 'menn' AND status = 1 AND date_added > DATE_SUB(now(), INTERVAL 10 DAY) ORDER BY visited DESC LIMIT 50"); //SELECT * ÞÝÐIR SELECT ALL
if($sql){
    while ($row=mysql_fetch_array($sql)) {
        $id = $row{"id"};
...
<div id="main"> 
    <ul id="products">
                <div class="category-info">
                  <div class="max-width">
                  <h1> <?php echo 'Vinsælast'; ?> </h1>
                  <p> Hér eru mest skoðuðu flíkurnar á Nomo seinustu 10 dagana.</p>
                  </div>
                </div>
           <div id="feedname">

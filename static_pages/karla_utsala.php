...
$count_sql = mysql_query("SELECT * FROM products WHERE category = 'menn' AND status = 1 AND dcount > 0"); //SELECT * ÞÝÐIR SELECT ALL
...
if($pages !=1) {
  ...
  if($page != 1){
    $prev = $page - 1;
    $pagination .= '<a href="/karlar_utsala/'.$prev.'"> < </a>';
  }else {
    $pagination .= '<';
  }
  if($pages > 1) {
    for($i = 1; $i <= $pages; $i++) {
        ...
        $pagination .= '<a href="/karlar_utsala/'.$i.'" class="'.$active.'"> '.$i.' </a>';
    }
  }
  if($page != $pages){
    $next = $page + 1;
    $pagination .= '<a href="/karlar_utsala/'.$next.'"> > </a>';
  }else {
    $pagination .= '>';
  }
...
$dynamic_list = "";
$sql = mysql_query("SELECT * FROM products WHERE category = 'menn' AND status = 1 AND dcount > 0 ORDER BY date_added DESC $limit"); //SELECT * ÞÝÐIR SELECT ALL
if($sql){
    while ($row=mysql_fetch_array($sql)) {
        $id = $row{"id"};
